...
var reverseExceptions = {};
for (var e in exceptions) {
    reverseExceptions[reverse(e)] = e;
}
...
function fixStem(stem, ending, suffix, options) {
	...
	if (exceptionRules[whole]) {
	    ...
	    if ((options.mood === "subjunctive" && options.tense === "present" && exceptionRules[whole]["subj stem"]) &&
	        ...
	        switch (type) {
	            case "e-ie":
	                stem = stem.replace("e", "ie"); // only replace the first one
	                break;
	            case "e-i":
...
var inflect = function(verb, options) {
	...
	// start at 3, because we have to have the ending (ir, ar, er) plus at
	// least one more letter. ie. we cannot match just the ending "ir" for example
	for (var i = 3; i < rev.length-1; i++) {
	    base = reverseExceptions[rev.substring(0, i)];
	    if (base) {
	        original = verb;
	        prefix = verb.substring(0, verb.length-i);
	        verb = base;
	        stem = base.substring(0, verb.length-2);
	        break;
	    }
	}
