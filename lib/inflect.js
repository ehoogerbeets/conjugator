...
function fixStem(stem, ending, suffix, options) {
    ...
    var mood = options.mood || "indicative";
    var tense = options.tense || "present";
    var positivity = options.positivity || "affirmative";
	var useSubjunctive = (mood === "subjunctive") ||
        (mood === "imperative" &&
	...
	if (exceptionRules[whole]) {
	    ...
	    if (useSubjunctive) {
            var subjStemAll = exceptionRules[whole].yo && exceptionRules[whole].yo !== "-ío";
            if (tense === "present" &&
